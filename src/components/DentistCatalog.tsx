...
import { DentistJson, DentistItem, RatingSummaryMap } from "../../interface"
import FilterBar from "./FilterBarDentist"
import getRatingSummary from "@/libs/getRatingSummary"
...
export default async function DentistCatalog({
    ...
    const [DentistJsonReady, summaryRes] = await Promise.all([
        dentistsJson,
        getRatingSummary().catch(() => ({ success: false, data: {} }))
    ]);
    const summary = summaryRes.data as RatingSummaryMap | undefined;
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 w-full max-w-7xl">
                {DentistJsonReady.data.length > 0 ? (
                    DentistJsonReady.data.map((Dentist: DentistItem) => {
                        ...
                        const ratingInfo = summary?.[Dentist._id];
